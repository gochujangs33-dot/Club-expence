...
- **클럽별 정산이력 탭** (관리자):
  ...
  - 마지막 확인 이후 새 정산은 해당 클럽에 `신규 등록 N건`으로 표시하며, `정산 이력 상세·수정`을 열면 읽음 처리
  - 행사별 참석 인원 숫자를 누르면 이름·사번 명단을 확인·복사 가능
  - 수정·삭제·영수증은 각 클럽 하단 `정산 이력 상세·수정`에서 제공
...
- **일반 사용자 정산 이력(v1.6.258)**: 상세 내역에서 비용 항목명과 함께 행사비·시설 및 장비 이용료·상품비 카테고리를 표시하고, 클럽별 그룹·접기/펼치기·올해 사용 요약 버튼을 관리자 화면과 동일하게 제공. 참석자 명단은 `참석자 명단 (N명)`을 눌렀을 때만 이름·사번이 펼쳐진다.
...
현재 버전: **v1.6.260** (`sw.js` `APP_VERSION`)
