...
6. **버그 수정 코드 — 재수정·롤백 금지** ⛔ (아래 4개 항목은 각각 재발 버그를 잡은 fix임)
   ...
   | `_calcCorpForItem` 시설비 계산 | 시설·장비는 **항상 인당 `rules.facilityLimit`(기본 85,000원) × 참석 인원수** flat 한도만 적용 — 행사비 4구간 공식과 절대 풀링/합산하지 말 것(v1.6.228~229에 풀링 시도했다가 v1.6.232에서 "시설비는 인원수와 무관한 정액 비용"이라는 결론으로 최종 롤백됨) | v1.6.232 | 시설비가 인당 구간표에 잘못 얽혀 행사비 유무에 따라 결과가 달라짐 |
...
3. 버전 상수 2곳을 함께 1 증가 (현재 1.6.232) — **아래 두 곳 모두 빠짐없이 bump**:
   - `sw.js` 의 `APP_VERSION`
...
현재 버전: **v1.6.232** (`sw.js` `APP_VERSION`)
