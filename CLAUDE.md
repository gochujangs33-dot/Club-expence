...
- **클럽별 정산이력 탭** (관리자):
  - 클럽 선택 드롭다운 없이, 등록된 모든 클럽을 기본 접힘 그룹으로 표시 (v1.6.259)
  - "정산인 선택" 드롭다운: 전체 이력 기준 정산인이 **2명 이상일 때만 표시**, 1명이면 자동 숨김
  - 마지막 확인 이후 새 정산은 해당 클럽에 `신규 등록 N건`으로 표시하며, 클럽을 펼치면 읽음 처리
  - 각 항목에 "정산인: OOO" 표시 (`entry.creatorName`)
  - 삭제는 관리자만 가능 — globalHistory + 작성자 개인 이력 + 참석자 누적 카운트 동시 처리
  - 전체 보기의 각 클럽 그룹에서 `사용 요약`을 열어 연간 행사별 비용 표를 확인 가능 (v1.6.256)
...
현재 버전: **v1.6.259** (`sw.js` `APP_VERSION`)
