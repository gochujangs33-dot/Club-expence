...
6. **버그 수정 코드 — 재수정·롤백 금지** ⛔ (아래 4개 항목은 각각 재발 버그를 잡은 fix임)
   ...
   | `_calcCorpForItem` 시설비 기본값 | 미승인 시설비는 **행사비와 같은 "인당 지원 한도 풀"에 합산**해서 marginal-delta 계산 — 이 항목 하나만 단독으로 계산(다른 항목과 미합산)하는 방식으로 되돌리기 금지 | v1.6.229 | 이미 다른 행사비로 자부담 구간이 올라간 상태에서도 새 시설비 항목마다 매번 100% 법인카드로 잘못 제안됨 |
...
3. 버전 상수 2곳을 함께 1 증가 (현재 1.6.229) — **아래 두 곳 모두 빠짐없이 bump**:
   - `sw.js` 의 `APP_VERSION`
...
- **시설·장비(FACILITY) 법인카드 제안 — 기본값 vs 승인 증액(v1.6.228~229)**: 카테고리 선택 즉시 뜨던
  팝업은 제거됨. 기본(미승인) 상태는 **행사비(EVENT)와 같은 "인당 지원 한도 풀"에 합산**해서 계산한다
  (이번 세션의 다른 EVENT/미승인 FACILITY 항목과 함께 marginal-delta 계산 — v1.6.228 최초 배포 때는
  이 항목 하나만 단독으로 계산했었는데, 이미 다른 행사비로 자부담 구간이 올라간 상태에서도 새 시설비가
  매번 100% 법인카드로 제안되는 문제가 있어 v1.6.229에서 풀링 방식으로 수정). 항목 추가 폼에 별도로
  뜨는 "이사진 승인 한도 증액" 체크박스를 체크하면 확인 팝업(`showFacilityApprovalModal`)이 뜨고,
  예 → 인당 `rules.facilityLimit`(기본 85,000원) × 인원수까지 법인카드 제안액 증액(이 풀과 무관한
  독립 flat 한도), 아니오 → 체크박스 자동 해제 + 기본값(풀링 방식)으로 원복. 승인 여부는 카테고리가
  아니라 `item.facilityApproved`에 항목별로 저장되어 수정 시 복원된다.
...
현재 버전: **v1.6.229** (`sw.js` `APP_VERSION`)
