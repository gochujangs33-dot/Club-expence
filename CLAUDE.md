...
- **차트 탭**: 월별 추이 / 클럽별 예산 소진율 / 카테고리 비중 / 클럽별 정산 횟수 / 행사별 참석 인원(v1.6.234, v1.6.236 가로 목록형, v1.6.245 최근 10건·왼쪽 레이블 항상 표시, v1.6.249 클릭 명단·복사) /
  클럽별 참석 인원 중복 제외(v1.6.234, v1.6.249 클릭 명단·복사) / 자부담 추이 — 전부 `maintainAspectRatio:false` +
...
- **클럽별 참석 인원(중복 제외) 차트(v1.6.234, v1.6.249)**: 고유 인원 판정은 사번(`employeeId`) 우선 — 동명이인은
  사번이 다르면 서로 다른 사람. 사번 없는 참석자만 이름 폴백. 올해(현재 연도) 이력만 집계, 많은 순 정렬. 막대 클릭 시 고유 참석자의 이름·사번 명단을 확인·복사.
- **클럽별 정산이력 탭** (관리자):
