...
3. sw.js 의 APP_VERSION 1 증가 (현재 1.6.220) — **코드 변경 시 bump 누락 절대 금지** (v1.6.202에서 누락 사고: 배포는 됐는데 전 유저가 구버전으로 인식)
...
- **행사비 인당 지원 한도 초과 확인(v1.6.220)**: (기존 법인카드 합계 + 이번 항목 법인카드) ÷ 인원이
  **정책 계산값**(구간표로 실제 행사비 계산한 인당 지원액)을 초과~85,000원 이내면 그 자리에서 이사진
  ...
  예산 자체는 초과 불가(하드캡). 시설·장비의 자체 사전승인 흐름(`showFacilityApprovalModal`, v1.6.142)과는
  별개.
...
현재 버전: **v1.6.220** (`sw.js` `APP_VERSION`)
