...
3. 버전 상수 2곳을 함께 1 증가 (현재 1.6.228) — **아래 두 곳 모두 빠짐없이 bump**:
   - `sw.js` 의 `APP_VERSION`
...
- **시설·장비(FACILITY) 법인카드 제안 — 기본값 vs 승인 증액(v1.6.228)**: 카테고리 선택 즉시 뜨던 팝업은
  제거됨. 기본(미승인) 상태는 이 항목 하나만 놓고 행사비와 동일한 인당 자부담 4구간 공식을 적용해 "인당
  지원 한도"를 제안한다. 항목 추가 폼에 별도로 뜨는 "이사진 승인 한도 증액" 체크박스를 체크하면
  확인 팝업(`showFacilityApprovalModal`)이 뜨고, 예 → 인당 `rules.facilityLimit`(기본 85,000원) ×
  인원수까지 법인카드 제안액 증액, 아니오 → 체크박스 자동 해제 + 기본값(구간표 방식)으로 원복. 승인
  여부는 카테고리가 아니라 `item.facilityApproved`에 항목별로 저장되어 수정 시 복원된다.
...
현재 버전: **v1.6.228** (`sw.js` `APP_VERSION`)
