...
## 5-4. 시설·장비(FACILITY) 항목 법인카드 제안 — 기본값 vs 승인 증액 (v1.6.228+)

시설·장비는 `calculate()`의 `eventCost`(4구간 공식 대상)에 애초에 포함되지 않는다 — 이 규칙 자체는
변경 없음. 단, `_calcCorpForItem`이 이 카테고리의 항목에 **법인카드 제안액**을 계산할 때는 아래 두
값 중 하나를 쓴다:

- **기본값(미승인)**: 이 항목의 금액을 인원수로 나눈 값(`amount / memberCount`)에 **행사비와 동일한
  인당 자부담 4구간 공식**(`calculateSelfPayPerPerson`)을 그대로 적용해 "이 항목 하나만 놓고" 인당
  지원 한도를 구한다 — 다른 항목의 `eventCost`와 합산(pool)하지 않는다.
- **승인 증액**(`_facilityApproved === true`, "이사진 승인 한도 증액" 체크박스로 확인 팝업을 거쳐 예로
  ...
  인당 85,000원 flat 한도(구간표 공식과 무관, 헤드카운트에 비례 스케일).
